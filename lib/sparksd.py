...
class SparksDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some sparksd methods
    ...
    @classmethod
    def from_sparks_conf(self, sparks_dot_conf):
        from sparks_config import SparksConfig
        config_text = SparksConfig.slurp_config_file(sparks_dot_conf)
        creds = SparksConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = sparkslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = sparkslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = sparkslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_sparksd_proto_version_with_sentinel_ping)
