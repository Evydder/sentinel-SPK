...
def is_valid_sparks_address(address, network='mainnet'):
    ...
    sparks_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Sparks address
        return False

    if (address_version != sparks_version):
        return False
...
# shims 'til we can fix the sparksd side
def SHIM_serialise_for_sparksd(sentinel_hex):
    from models import SPARKSD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = SPARKSD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in sparksd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # sparksd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    sparksd_hex = serialise(obj)
    return sparksd_hex


# shims 'til we can fix the sparksd side
def SHIM_deserialise_from_sparksd(sparksd_hex):
    from models import SPARKSD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(sparksd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('sparks.conf')
