...
def has_sparks_conf():
    ...
    valid_sparks_conf = False

    # ensure sparks_conf exists & readable
    #
    # if not, print a message stating that Sparks Core must be installed and
    # configured, including JSONRPC access in sparks.conf
    try:
        f = io.open(config.sparks_conf)
        valid_sparks_conf = True
    except IOError as e:
        print(e)

    return valid_sparks_conf
...
def main():
    ...
    if not has_sparks_conf():
        print("SparksCore must be installed and configured, including JSONRPC access in sparks.conf")
        sys.exit(1)
