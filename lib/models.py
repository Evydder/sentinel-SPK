...
class GovernanceObject(BaseModel):
    ...
    # sync sparksd gobject list with our local relational DB backend
    @classmethod
    def sync(self, sparksd):
        golist = sparksd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_sparksd(sparksd, item)
    ...
    @classmethod
    def import_gobject_from_sparksd(self, sparksd, rec):
        import decimal
        import sparkslib
        import inflection
        ...
        # shim/sparksd conversion
        object_hex = sparkslib.SHIM_deserialise_from_sparksd(object_hex)
        objects = sparkslib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from sparksd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from sparksd, with every run
        ...
        # get/create, then sync payment amounts, etc. from sparksd - Sparksd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(sparksd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from sparksd! %s" % e)
            govobj.vote_delete(sparksd)
            return (govobj, None)
    ...
    def vote_delete(self, sparksd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(sparksd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, sparksd, signal, outcome):
        import sparkslib
        ...
        output = sparksd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = sparkslib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(sparksd, signal)

    def sync_network_vote(self, sparksd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = sparksd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = SPARKSD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 sparks addr, non-multisig
            if not sparkslib.is_valid_sparks_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Sparks address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (sparkslib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SparksDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import sparkslib
        obj_data = sparkslib.SHIM_serialise_for_sparksd(self.serialise())
    ...
    def prepare(self, sparksd):
        try:
            object_hash = sparksd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = SPARKSD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not sparkslib.is_valid_sparks_address(addr, config.network):
    ...
    def hash(self):
        import sparkslib
        return sparkslib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SPARKSD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, sparksd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - sparksd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, sparksd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - sparksd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, sparksd):
        now = int(time.time())
        return (self.created_at < (now - sparksd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, sparksd):
        if self.is_expired(sparksd):
            return False

        return True

    def is_deletable(self, sparksd):
        if self.is_expired(sparksd):
            return True
